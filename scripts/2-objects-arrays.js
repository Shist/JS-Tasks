...
let objectsArr = [];
vegetables.forEach((item, i) => {
  objectsArr.push({ word: item, length: item.length });
});
console.log("Полученный массив объектов:");
objectsArr.forEach((item, i) => {
  console.log(`arr[${i}] = { word: "${item.word}", length: ${item.length} }`);
});
console.log("Статистика по полученному массиму объектов:");
objectsArr.forEach((item, i) => {
